...
public class Validation {
    ...
    // @NotEmpty: 문자열 타입에서 null과 빈문자열("")을 허용치 않음
    @NotEmpty
    ...
    // client로부터 데이터를 받을 때는 기본형 데이터타입을 쓰지않는 것이 좋다(int)
    @NotNull
    // @NotBlank - 문자열만 검사함, Integer에선 사용불가능
    private Integer number;

    // @Length(min=?,max=?) :  문자열 타입에서 길이의 최소최대를 지정함
    // null이면 검사하지 않음
    @Length(min = 4)
    @NotNull
