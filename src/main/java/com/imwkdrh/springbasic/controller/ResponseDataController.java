...
public class ResponseDataController {
    ...
    @GetMapping("/{name}")
    public ResponseDto responseBodyMethod(@PathVariable("name") String name) {
    ...
    // ResponseEntity 클래스는 제너릭으로 response body 데이터의 타입을 전달해야함ㅠ
    @GetMapping("/response-entity/{name}")
    public ResponseEntity<ResponseDto> responseEntity(@PathVariable("name") String name) {
        ResponseDto response = new ResponseDto(name, new Date());
