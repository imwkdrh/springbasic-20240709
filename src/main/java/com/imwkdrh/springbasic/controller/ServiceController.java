...
import com.imwkdrh.springbasic.service.BasicService;
...
public class ServiceController {
    ...
    // - 생성자를 통한 의존성 주입, setter를 통한 의존성 주입, 필드를 통한 의존성 주입
    // - spring framework에서 권장하고 있는 의존성 주입 방법은 생성자를 통한 의존성 주입(의존성이 주입되지 않은 상태가 존재할 수
    // 없기 때문)
    // - spring framework에서 IoC를 통해 의존성을 주입할 땐 주입할 인스턴스의 클래스가 Spring bean으로 등록되어있어야
    // 함

    // private BasicService basicService = new BasicServiceImplement();

    // @Autowired : 등록된 spring bean을 제어 역전을 통해 의존성 주입을 하수 있도록 하는 어노테이션
    // 필드 객체
    // @Autowired
